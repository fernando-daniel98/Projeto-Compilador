...
int main(int argc, char *argv[]){	
    ...
    for(int i=1; i<argc; i++){
        ...
        if((strcmp(argv[i], "-V") == 0) || (strcmp(argv[i], "-v") == 0)){
            flagVerbose = 1;
            arquivoSaida = fopen("output/lexerAndParserResults.txt", "w");
        ...
        // Para gerar o código intermediário em um arquivo
            ...
            flagCI = 1;
            arquivoSaida_Intermediario = fopen("output/intermediateCode.txt", "w");
        ...
        // Argumento para gerar o código assembly em um arquivo
            ...
            flagCA = 1;
            arquivoSaida_Assembly = fopen("output/assemblyCode.txt", "w");
    ...
    // SEMPRE executa a análise semântica, mesmo com erros léxicos/sintáticos
    if(arvoreSintatica != NULL) {
        ...
        if(procuraTabelaExp(tabelaHash, "main", "global", AtivK) == NULL){
            mostrarErroSemantico(FuncMainNaoDeclarada, "main", lineNum);
        }
    } else {
    ...
    /* Caso tenha algum erro (lexico, sintatico ou semantico), nao criar codigo intermediario*/
    int totalErros = lexical_errors + syntax_errors + teveErroSemantico;
    if(totalErros > 0){    
        ...
        printf("==========================================\n");
        printf("Nao foi possivel gerar o codigo intermediario devido aos erros encontrados.\n");
    ...
    if(!arquivoSaida_Binario){
        arquivoSaida_Binario = fopen("output/binaryCode.txt", "w");
        if(arquivoSaida == NULL){
            printf(ANSI_COLOR_RED "Erro:" ANSI_COLOR_RESET);
            printf("Nao foi possivel criar o arquivo de saida.\n");
